...
class ppa_analyser : public analyser {
 public:
  ppa_analyser(const YAML::Node &configs) : analyser(configs, 2){};
  ~ppa_analyser() override = default;
 ...
 private:
  ...
  void open_writers();
  void gen_headers();
  nlohmann::json ppa_analyse(const std::vector<std::shared_ptr<Path>> &paths);
  void match(
